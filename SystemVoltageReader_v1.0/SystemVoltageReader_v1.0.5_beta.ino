...
const int calPin = D5;      // Calibration pin
const float knownResistance = 470.0; // Known resistor value (Ohms)
...
void setup() {
  ...
  // Initialize button, range selection and cal pin
...
void loop() {
  ...
  if (digitalRead(buttonPin) == LOW) {
    ...
    if (digitalRead(calPin) == LOW) {
      ...
      while (measuredResistance <= 0) {
        Serial.print("Enter measured resistance for a known resistor of ");
        Serial.print(knownResistance);
        Serial.print(" Ohms: ");
        while (!Serial.available());
        measuredResistance = Serial.parseFloat();
      }
      // Calculate the calibration factor
      calFactor = measuredResistance / knownResistance;
