...
void loop() {
  ...
  digitalWrite(NodeMCU_BUILTIN_LED, HIGH); // When the pin is 1/HIGH, the LED is off
  delay(5000); // interval of 5s between each update
...
long long getLocalUnixTimestamp() {
  long long timestamp_long = strtoll(timestamp.c_str(), &pEnd, 10) / 1000;
...
String updateTime() {
  long long timestamp_long = strtoll(String((unix_timestamp.toInt() + (millis()/1000)) - 17).c_str(), &pEnd, 10);
