...
// Replace with the IP address and port of your Node-RED API server
...
// Replace with the chosen endpoint of your Node-RED API server
String endpoint = "/timeserver_all";

// Replace with the correct timezone_offset for your location
...
void loop() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    if (boot_time == 0) {
      ...
      current_time = getCurrentTimeHHMMSS();
      Serial.print("Sending HTTP GET request to: http://");
      Serial.print(String(NODERED_SERVER));
      Serial.print(":");
      Serial.print(String(NODERED_PORT));
      Serial.print(endpoint);
      Serial.print("\nAPI endpoint response: ");
      Serial.println(api_response);
      
      Serial.print("Servers current time: ");
    ...
    if (booted != true) {
      boot_unix_timestamp = String(getLocalUnixTimestamp());
      Serial.print("Boot unix_timestamp: ");
      Serial.println(boot_unix_timestamp);
      booted = true;
    }
  ...
  Serial.print("Locally updated time: ");
  Serial.println(updated_time);
...
String getTimestamp(String json) {
  ...
  int endIndex = json.indexOf("timestamp") + 24; // end index of timestamp value
...
long long getLocalUnixTimestamp() {
  ...
  // Check the sign of the "timezone_offset" variable. If less than 0, the subtraction is performed, otherwise, the addition is performed.
  if (timezone_offset < 0) {
      return timestamp_long - abs(adjustment);
    } else {
...
String updateTime() {
  long long timestamp_long = strtoll(String((boot_unix_timestamp.toInt() + (millis()/1000)) - boot_time).c_str(), NULL, 10);
  time_t now = timestamp_long;
  char time[20];
