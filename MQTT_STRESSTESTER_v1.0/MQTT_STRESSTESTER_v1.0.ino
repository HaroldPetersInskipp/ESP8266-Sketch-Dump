...
const char* esp8266unitNumber = "ESP8266Client_3";  // Set a esp8266unitNumber to identify each unit
...
const char* MQTT_PUB_TOPIC = "ESP8266/HEARTBEAT";// Set the MQTT publishing topic to use
...
const int buttonPin8 = D8;          // Button 8 input pin (GPIO15)
// const int buttonPin9 = D9;          // Button 9 input pin (GPIO??)
...
void setup() {
  ...
  Serial.println("Pin: 'D8' as button 8");
  //pinMode(buttonPin9, INPUT_PULLUP);
  //Serial.println("Pin: 'D9' as button 9");
...
void loop() {
  ...
  if (digitalRead(buttonPin8) == LOW) {
    digitalWrite(NodeMCU_BUILTIN_LED, LOW); // When the pin is 0/LOW, the LED is on
    const char* placeholder = "MQTT_HEARTBEAT_STRESSTEST";
    // Publish an MQTT message on topic
    uint16_t packetIdPub1 = mqttClient.publish(MQTT_PUB_TOPIC, 2, false, placeholder);
    Serial.println();
    Serial.printf("Publishing a MQTT message on topic '%s' at QoS 2, packetId: %i\n", MQTT_PUB_TOPIC, packetIdPub1);;
